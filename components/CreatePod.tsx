...
import { ArrowLeft, Plus, X, ArrowRight, ArrowUp, ArrowDown, Undo2, Calendar, User, Tag, Lightbulb, PenTool, Monitor, Server, Bug, Megaphone, Settings } from 'lucide-react';
...
const roleOptions = ['Product', 'Design', 'Frontend', 'Backend', 'QA', 'Marketing', 'Operations'];
...
export function CreatePod({ user, onBack, onCreatePod }: CreatePodProps) {
  ...
  const [workflowOrder, setWorkflowOrder] = useState<string[]>(['Product', 'Design', 'Frontend', 'Backend', 'QA']);
  const [workflowHistory, setWorkflowHistory] = useState<string[][]>([['Product', 'Design', 'Frontend', 'Backend', 'QA']]);
  ...
  const handleAddMember = () => {
    if (!selectedEmployee || !selectedRole || !taskDescription || !memberStartDate || !memberEndDate) {
      setErrors({
    ...
    const employee = companyEmployees.find(emp => emp.id === selectedEmployee);
    if (!employee) return;
    ...
    const newMember: TeamMember = {
      id: `${selectedEmployee}-${Date.now()}`, // Allow multiple instances of same employee
      name: employee.name,
      role: selectedRole,
  ...
  const handleRemoveMember = (memberId: string) => {
    ...
    if (memberToRemove) {
      const remainingMembersWithRole = members.filter(m => m.id !== memberId && m.role === memberToRemove.role);
  ...
  const handleUpdateMember = (memberId: string, field: string, value: string) => {
  ...
  const validateAndSubmit = () => {
    ...
    const missingMembers = workflowOrder.filter(role => !members.some(member => member.role === role));
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="py-4">
            ...
            <div>
              ...
              <p className="text-muted-foreground mt-1">Set up a new project handoff team</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  />
                  {errors.podName && (
                    <p className="text-red-500 text-sm mt-1">{errors.podName}</p>
                  )}
                </div>
                <div>
                  ...
                  />
                  {errors.podOwner && (
                    <p className="text-red-500 text-sm mt-1">{errors.podOwner}</p>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-2">
                  Choose whether this POD is for a new feature development or a go-live deployment
                </p>
              </div>
              ...
              <div>
                ...
                {errors.podDescription && (
                  <p className="text-red-500 text-sm mt-1">{errors.podDescription}</p>
                )}
              </div>
              ...
              <div className="border border-gray-200 rounded-lg">
                <div className="p-3 bg-gray-50 border-b border-gray-200">
                  <div className="flex justify-between items-center">
                    <h3 className="text-gray-900">Team Members ({members.length})</h3>
                  </div>
                </div>
                            ...
                            <div className="flex-1 space-y-2">
                              ...
                              <div>
                                ...
                                <Textarea
                                  ...
                                  onChange={(e) => handleUpdateMember(member.id, 'taskDescription', e.target.value)}
                                  className="mt-1"
                                  rows={2}
                                />
                              </div>
          ...
          <Card className="p-4">
            ...
            <p className="text-sm text-gray-600 mb-3">
              Define the sequence of handoffs. Work will flow from left to right.
            </p>
                        ...
                        <div className="flex items-center space-x-2 min-w-fit">
                          ...
                          <div className="flex flex-col space-y-1">
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => handleMoveWorkflowItem(index, 'up')}
                              disabled={index === 0}
                            ...
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => handleMoveWorkflowItem(index, 'down')}
                    ...
                    {workflowOrder.map(role => {
                      ...
                      return (
                        <div key={role} className="flex justify-between items-center">
                          ...
                          <span className={roleMembers.length > 0 ? 'text-gray-900' : 'text-red-600'}>
